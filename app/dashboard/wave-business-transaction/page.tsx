...
import { Search, ChevronLeft, ChevronRight, ArrowUpDown, Copy, Eye, MoreHorizontal, CheckCircle, XCircle } from "lucide-react"
...
function WaveBusinessContent() {
  ...
  const handleSuccessSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = `${baseUrl.replace(/\/$/, "")}/api/payments/wave-business-transactions/${successTransaction.uid}/success/`
  ...
  const handleFailedSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = `${baseUrl.replace(/\/$/, "")}/api/payments/wave-business-transactions/${failedTransaction.uid}/failed/`
      await apiFetch(endpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => handleOpenDetail(transaction)} className="sm:hidden">
