...
export default function BettingTransactionsPage() {
  ...
  const handleProcessCancellation = async (approve: boolean) => {
    ...
    try {
      const payload = {
        success: approve,
        admin_notes: cancellationNotes || (approve ? "Cancellation approved by admin" : "Cancellation rejected by admin")
      }
  ...
  const handleRefundSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = `${baseUrl.replace(/\/$/, "")}/api/payments/betting/admin/transactions/${refundTransaction.uid}/refund-partner/`
      await apiFetch(endpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
    ...
    <>
      ...
      <AlertDialog open={cancellationModalOpen} onOpenChange={setCancellationModalOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={processingCancellation}>{t("common.cancel")}</AlertDialogCancel>
            <div className="flex gap-2">
              ...
              >
                {processingCancellation ? t("bettingTransactions.processing") : (t("common.approve") || "Approve")}
              </Button>
            </div>
          </AlertDialogFooter>
        </AlertDialogContent>
